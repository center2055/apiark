...
mod http_engine {
    ...
    async fn test_get_request() {
        ...
        println!("  GET /get: {} {}ms {}B", resp.status, resp.time_ms, resp.size_bytes);
    }

    #[tokio::test]
    async fn test_post_json() {
        let mut params = base_params(HttpMethod::POST, "https://httpbin.org/post");
        params.headers.push(KeyValuePair {
    ...
    async fn test_patch_request() {
        ...
        let resp = HttpEngine::send(params).await.expect("PATCH request failed");
        assert_eq!(resp.status, 200);
    ...
    async fn test_delete_request() {
        ...
        let resp = HttpEngine::send(params).await.expect("DELETE request failed");
        assert_eq!(resp.status, 200);
    ...
    async fn test_query_params() {
        ...
        params.params.push(KeyValuePair {
            key: "foo".into(),
            value: "bar".into(),
            enabled: true,
        });
        ...
        let resp = HttpEngine::send(params).await.expect("Query params request failed");
        let body: serde_json::Value = serde_json::from_str(&resp.body).unwrap();
    ...
    async fn test_custom_headers() {
        ...
        let resp = HttpEngine::send(params).await.expect("Headers request failed");
        let body: serde_json::Value = serde_json::from_str(&resp.body).unwrap();
    ...
    async fn test_form_urlencoded() {
        ...
        params.body = Some(RequestBody {
            ...
            content: String::new(),
            form_data: vec![
                KeyValuePair { key: "username".into(), value: "testuser".into(), enabled: true },
                KeyValuePair { key: "password".into(), value: "secret123".into(), enabled: true },
            ],
        });
    ...
    async fn test_redirect_follow() {
        ...
        let resp = HttpEngine::send(params).await.expect("Redirect request failed");
        assert_eq!(resp.status, 200);
    ...
    async fn test_redirect_no_follow() {
        ...
        let resp = HttpEngine::send(params).await.expect("No-follow redirect failed");
    ...
    async fn test_response_headers_and_cookies() {
        ...
        let resp = HttpEngine::send(params).await.expect("Response headers request failed");
        assert_eq!(resp.status, 200);
        let has_x_test = resp.headers.iter().any(|h| h.key.eq_ignore_ascii_case("x-test"));
    ...
    async fn test_cookie_override() {
        ...
        let resp = HttpEngine::send(params).await.expect("Cookie override request failed");
        let body: serde_json::Value = serde_json::from_str(&resp.body).unwrap();
...
mod auth_handlers {
    ...
    async fn test_basic_auth_wrong_password() {
        ...
        let resp = HttpEngine::send(params).await.expect("Bad auth request failed");
    ...
    async fn test_api_key_header() {
        ...
        let resp = HttpEngine::send(params).await.expect("API Key header auth failed");
        let body: serde_json::Value = serde_json::from_str(&resp.body).unwrap();
        let headers = body["headers"].as_object().unwrap();
    ...
    async fn test_api_key_query() {
        ...
        let resp = HttpEngine::send(params).await.expect("API Key query auth failed");
        let body: serde_json::Value = serde_json::from_str(&resp.body).unwrap();
        let args = body["args"].as_object().unwrap();
    ...
    fn test_aws_v4_signature() {
        ...
        assert!(extra_headers.iter().any(|(k, _)| k == "x-amz-content-sha256"));
    ...
    fn test_aws_v4_with_session_token() {
        ...
        assert!(extra_headers.iter().any(|(k, v)| k == "x-amz-security-token" && v == "session-token-123"));
    ...
    fn test_ntlm_negotiate() {
        let negotiate = apiark_lib::http::auth_handlers::generate_ntlm_negotiate("MYDOMAIN", "MYPC");
        // Should be valid base64
        use base64::Engine;
        let decoded = base64::engine::general_purpose::STANDARD.decode(&negotiate).unwrap();
        ...
        println!("  NTLM negotiate: valid Type 1 message ({} bytes decoded)", decoded.len());
    }
    ...
    fn test_ntlm_authenticate() {
        ...
        assert!(result.is_ok(), "NTLM Type 3 generation failed: {:?}", result.err());
        let type3_b64 = result.unwrap();
        let decoded = base64::engine::general_purpose::STANDARD.decode(&type3_b64).unwrap();
        assert_eq!(&decoded[0..8], b"NTLMSSP\0");
        assert_eq!(u32::from_le_bytes([decoded[8], decoded[9], decoded[10], decoded[11]]), 3);
        println!("  NTLM authenticate: valid Type 3 message ({} bytes)", decoded.len());
    }
}
...
mod scripting {
    ...
    fn test_pre_request_set_header() {
        ...
        assert_eq!(result.request.headers.get("X-Request-ID").unwrap(), "abc123");
        assert_eq!(result.request.headers.get("Authorization").unwrap(), "Bearer mytoken");
    ...
    fn test_globals_and_variables() {
        ...
        assert_eq!(result.global_mutations["apiVersion"].as_ref().unwrap(), "v3");
        assert_eq!(result.variable_mutations["requestId"].as_ref().unwrap(), "req_999");
...
mod assertions {
    ...
    fn test_response_time_assertion() {
        let resp = make_response();
        let assertions: serde_yaml::Value = serde_yaml::from_str("responseTime:\n  lt: 5000").unwrap();
        let results = evaluate_assertions(&assertions, &resp);
        assert!(results.iter().all(|r| r.passed));
    ...
    fn test_contains_assertion() {
        let resp = make_response();
        let assertions: serde_yaml::Value = serde_yaml::from_str("headers.content-type:\n  contains: json").unwrap();
        let results = evaluate_assertions(&assertions, &resp);
        assert!(results.iter().all(|r| r.passed));
...
mod mock_server {
    ...
    async fn test_mock_server_lifecycle() {
        ...
        let temp_dir = std::env::temp_dir().join(format!("apiark_mock_test_{}", std::process::id()));
...
"#,
        ...
        let resp = client.get(format!("http://127.0.0.1:{}/api/users", port))
            .send()
            .await
        ...
        println!("  Mock server: started on port {}, served /api/users correctly", port);
...
mod websocket {
    ...
    async fn test_websocket_connect_send_receive() {
        ...
        // Send a message
        ws.send(tungstenite::Message::Text("Hello ApiArk!".into())).await.unwrap();

        // Receive echo
        if let Some(Ok(tungstenite::Message::Text(text))) = ws.next().await {
            assert_eq!(text, "Hello ApiArk!");
            println!("  WebSocket echo: sent and received 'Hello ApiArk!' on localhost:{}", port);
        } else {
            panic!("Did not receive echo message");
        }

        // Send another message to verify ongoing connection
        ws.send(tungstenite::Message::Text("second message".into())).await.unwrap();
        if let Some(Ok(tungstenite::Message::Text(text))) = ws.next().await {
...
mod file_watcher {
    ...
    fn test_file_change_detection() {
        let temp_dir = std::env::temp_dir().join(format!("apiark_watcher_test_{}", std::process::id()));
        std::fs::create_dir_all(&temp_dir).unwrap();

        let test_file = temp_dir.join("test-request.yaml");
        std::fs::write(&test_file, "name: Original\nmethod: GET\nurl: http://example.com\n").unwrap();
        ...
        watcher.watch(&temp_dir, RecursiveMode::Recursive).expect("Failed to watch directory");

        // Modify the file
        std::thread::sleep(Duration::from_millis(100));
        std::fs::write(&test_file, "name: Modified\nmethod: POST\nurl: http://example.com/updated\n").unwrap();

        // Wait for event (with timeout)
        let event = rx.recv_timeout(Duration::from_secs(5));
        assert!(event.is_ok(), "No file change event received within 5 seconds");
        let event = event.unwrap();
        assert!(!event.paths.is_empty());
        println!("  File watcher: detected change to {:?} (kind: {:?})", event.paths[0].file_name(), event.kind);
    ...
    fn test_file_delete_detection() {
        let temp_dir = std::env::temp_dir().join(format!("apiark_watcher_del_{}", std::process::id()));
        std::fs::create_dir_all(&temp_dir).unwrap();

        let test_file = temp_dir.join("to-delete.yaml");
        std::fs::write(&test_file, "name: ToDelete\nmethod: GET\nurl: http://example.com\n").unwrap();
        ...
        watcher.watch(&temp_dir, RecursiveMode::Recursive).expect("Failed to watch");
        ...
        println!("  File watcher: detected file deletion (kind: {:?})", event.unwrap().kind);
...
mod storage {
    ...
    fn test_yaml_request_roundtrip() {
        let temp_dir = std::env::temp_dir().join(format!("apiark_storage_test_{}", std::process::id()));
...
"#;
        ...
        assert_eq!(parsed["variables"]["baseUrl"].as_str().unwrap(), "http://localhost:3000");
...
mod history {
    ...
    fn test_history_db_operations() {
        ...
        // Search
        let mut stmt = conn.prepare("SELECT method, url, status FROM history WHERE url LIKE '%example%'").unwrap();
        let entry: (String, String, i64) = stmt.query_row([], |r| {
